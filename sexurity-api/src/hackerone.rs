...
impl HackerOneClient {
    pub fn new(csrf_token: String, session_token: String) -> Self {
        ...
        headers.insert("x-csrf-token", header::HeaderValue::from_str(&csrf_token).unwrap());
        headers.insert("cookie", header::HeaderValue::from_str(&cookie_header).unwrap());
...
pub fn get_hackerone_csrf_token(session_token: &str) -> Result<String, Box<dyn Error>> {
    ...
    let http_response = client.get("https://hackerone.com/bugs")
