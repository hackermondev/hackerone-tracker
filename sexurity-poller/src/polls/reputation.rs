...
use sexurity_api::redis::{redis::cmd, redis, save_vec_to_set, load_set_to_vec};
...
pub fn run_poll(config: &PollConfiguration) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let last_run_time: Option<String> = cmd("GET").arg("reputation_poll_last_run_time").query(&mut redis_conn)?;
    ...
    if last_run_time.is_none() || last_rep_data.is_none() {
        // first run
        save_vec_to_set("reputation_poll_last_data".to_string(), data, &mut redis_conn)?;
...
fn set_last_run_time_now(conn: &mut redis::Connection) {
    ...
    cmd("SET").arg("reputation_poll_last_run_time").arg(ms).query::<String>(conn).unwrap();
}

fn get_reputation_data(handle: &str, client: &HackerOneClient) -> Result<Vec<RepData>, Box<dyn std::error::Error>> {
    ...
    let researchers = data.data.unwrap().selected_team.unwrap().participants.unwrap().edges.unwrap();
...
fn get_old_reputation_data(conn: &mut redis::Connection) -> Option<Vec<RepData>> {
    let last_rep_data = load_set_to_vec(String::from("reputation_poll_last_data"), conn).unwrap_or(vec![]);
